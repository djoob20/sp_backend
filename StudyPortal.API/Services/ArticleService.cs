...
public class ArticleService : AbstractService, ISubjectsService<Article>
{
    ...
    public ArticleService(IOptions<StudyPortalDatabaseSettings> studyPortalDatabaseSettings) : base(
        studyPortalDatabaseSettings)
    {
        _articleCollection =
            mongoDatabase.GetCollection<Article>(studyPortalDatabaseSettings.Value.ArticlesCollectionName);
    }

    //Return all articles
    public async Task<List<Article>> GetAsync()
    {
        return await _articleCollection.Find(_ => true).ToListAsync();
    }

    //Get an article with the given Id
    public async Task<Article?> GetAsync(string id)
    {
        return await _articleCollection.Find(x => x.Id == id).FirstOrDefaultAsync();
    }
