...
public class ArticleController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllArticles()
    {
        try
        {
            ...
            if (articles.Any()) return Ok(articles);
    ...
    public async Task<IActionResult> UpdateArticle(string id, Article updatedArticle)
    {
        try
        {
            if (!id.Equals(updatedArticle.Id)) return BadRequest("Course ID does mismatch");

            var articleToUpdate = await _articleService.GetAsync(id);
            if (articleToUpdate == null) return NotFound($"Course with Id = {id} not found");
    ...
    public async Task<IActionResult> DeleteArticle(string id)
    {
        try
        {
            ...
            if (article == null) return NotFound($"Course with Id = {id} not found");
