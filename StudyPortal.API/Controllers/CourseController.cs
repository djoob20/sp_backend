...
public class CourseController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllCourses()
    {
        try
        {
            ...
            if (courses.Any()) return Ok(courses);
            return NotFound();
        }
        catch (Exception)
        {
    ...
    public async Task<IActionResult> UpdateCourse(string id, Course updatedCourse)
    {
        try
        {
            if (!id.Equals(updatedCourse.Id)) return BadRequest("Course ID does mismatch");

            var courseToUpdate = await _courseService.GetAsync(id);
            if (courseToUpdate == null) return NotFound($"Course with Id = {id} not found");
