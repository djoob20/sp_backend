...
public class CourseControllerTest
{
    ...
    {
        //Arrange
        var courseId = SubjectsFixture.GetTestCourses().First().Id;
        var expectedCourse = SubjectsFixture.GetTestCourses().First();
        var courseServiceMock = new Mock<ISubjectsService<Course>>();
        courseServiceMock.Setup(service => service.GetAsync(courseId == null ? "" : courseId))
    ...
    public async Task Test_UpdateCourse_OnSuccess_Returns_StatusCode_204()
    {
        ...
        //Act
        var result =
            await controller.UpdateCourse(courseId == null ? "" : courseId, SubjectsFixture.GetTestCourses().First());
